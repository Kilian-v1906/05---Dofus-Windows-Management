...
class DofusWindowManager(QMainWindow):
    ...
    def refresh_windows(self):
        # Recherche des fenêtres Dofus (fichiers dofus.exe uniquement)
        ...
        def callback(hwnd, extra):
            if win32gui.IsWindowVisible(hwnd):
                ...
                try:
                    process_path = win32process.GetModuleFileNameEx(win32process.OpenProcess(0x0400 | 0x0010, False, pid), 0)
                    print(os.path.basename(process_path).lower())
                    if os.path.basename(process_path).lower() == "dofus.exe":
                        title = win32gui.GetWindowText(hwnd)
                        self.add_window_item(title)
                except Exception:
                    pass  # Certaines fenêtres peuvent ne pas avoir d'accès
    ...
    def add_window_item(self, window_title):
        ...
        container = QWidget()
        container.setProperty("dragging", False)
        ...
        # Drag-and-drop
        def start_drag():
            container.setProperty("dragging", True)
        ...
        drag_button.pressed.connect(start_drag)
        drag_button.released.connect(end_drag)
